...
function ParentHyperbole({ onBack, commonColor, buttonColor, discoveredColor, createTextColor, addOrangeButton }) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && !notesDataLoaded) {
      ...
      loadStoredData('HyperboleNotes')
        .then(data => {
          setEditorContent(data || '');
          console.log('Data loaded for HyperboleNotes:', data);
          setNotesDataLoaded(true); // Mark data as loaded
        })
        .catch(err => console.error('Error loading HyperboleNotes:', err));
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    if (!editorContent.trim()) {
      Alert.alert('Uh oh, sire,', 'Editor content is empty!');
      return;
    }
    saveData('HyperboleNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert(
          'Well done, wordsmith or wordsmithesse!',
          'Your beautiful hyperbole notes have been successfully stored for safekeeping!',
        );
      })
          ...
          <View style={{position: 'relative', bottom: '5%'}}>
            ...
            <View
              style={{
                ...
                borderRadius: '50%',
                left: '32.5%',
                top: isMediumTallPhone ? '22%' : isCompactMediumPhone ? '21.5%' : isSmallPhone ? '19.5%' : isGalaxySPhone ? '24.5%' : null,
              }}>
              <TouchableOpacity
                style={{
            ...
            <View style={{bottom: '13%'}}>
              ...
              <View
                style={{
                  position: 'relative',
                  right: '40%',
                  top: isMediumTallPhone ? '16%' : isCompactMediumPhone ? '14%' : isSmallPhone ? '14%' : isGalaxySPhone ? '18%' : null, 
                  bottom: '6%',
                  marginTop: '4%',
                }}>
                <View
          ...
          <View style={{height: '104%'}}>
            <View
              style={{
                ...
                padding: '2.5%',
                fontSize: wp(4.5),
                marginBottom: isMediumTallPhone ? hp(0.25) : isCompactMediumPhone ? hp (0.1) : isGalaxySPhone ? hp(0.25) : null,
                backgroundColor: 'white',
                borderRadius:wp(4),
                bottom: '1.75%',
              }}>
              <Text> Write your hyperbole notes and thoughts here</Text>
            </View>
            <View style={{ height: isMediumTallPhone ? hp(62) : isCompactMediumPhone ? hp(60) : isSmallPhone ? hp(55.5) : isGalaxySPhone ? hp(62) : null }}>            
            <DeviceQuill
            ...
            <View
              style={{
                position: 'relative',
                bottom: isMediumTallPhone ? '2.50%' : isCompactMediumPhone ? '4.0%' : isSmallPhone ? '4%' : isGalaxySPhone ? '2.50%' : null,
                left: '50%',
                width: '50%',
            ...
            <View style={{ position: 'relative', right: '35%', bottom: isMediumTallPhone ? '9.15%' : isCompactMediumPhone ? '11.5%' : isSmallPhone ? '12.5%' : isGalaxySPhone ? '9.15%' : null }}>
