...
function ParentImagery({onBack, commonColor, buttonColor, discoveredColor, createTextColor, addOrangeButton}) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && !notesDataLoaded) {
      ...
      loadStoredData('ImageryNotes')
        .then(data => {
          setEditorContent(data || '');
          console.log('Data loaded for ImageryNotes:', data);
          setNotesDataLoaded(true); // Mark data as loaded
        })
        .catch(err => console.error('Error loading ImageryNotes:', err));
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    if (!editorContent.trim()) {
      Alert.alert(
        'Easy now, young buck!',
        "Can't save if there's nothing to save. Catchin' my drift?",
      );
      return;
    }
    saveData('ImageryNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert(
          'Bouncy!',
          'Your imagery notes have been successfully saved, you brilliant wordsmithesse or wordsmith!',
        );
      })
          ...
          <View style={{position: 'relative', bottom: '5%'}}>
            ...
            <View
              style={{
                ...
                borderRadius: '50%',
                left: '32.5%',
                top: isMediumTallPhone ? '22%' : isCompactMediumPhone ? '18%' : isSmallPhone ? '14%' : isGalaxySPhone ? '23.5%' : null,
              }}>
              <TouchableOpacity
                style={{
            ...
            <View style={{bottom: '13%'}}>
              ...
              <View
                style={{
                  position: 'relative',
                  right: '40%',
                  top: isMediumTallPhone ? '16%' : isCompactMediumPhone ? '14%' : isSmallPhone ? '14%' : isGalaxySPhone ? '15%' : null, 
                  bottom: '6%',
                  marginTop: '4%',
                }}>
                <View
        ...
        <>
          <View style={{height: '104%'}}>
            <View
              ...
              <Text> Write your imagery notes and thoughts here</Text>
            </View>
            <View style={{ height: isMediumTallPhone ? hp(62) : isCompactMediumPhone ? hp(60) : isSmallPhone ? hp(58) : isGalaxySPhone ? hp(66) : null }}>            
            <DeviceQuill
            ...
            <View
              style={{
                position: 'relative',
                bottom: isMediumTallPhone ? '2.33%' : isCompactMediumPhone ? '4.0%' : isSmallPhone ? '4.0%' : isGalaxySPhone ? '3.25%' : null,
                left: '50%',
                width: '50%',
            ...
            <View style={{position: 'relative', right: '35%', bottom: isMediumTallPhone ? '9.00%' : isCompactMediumPhone ? '11.5%' : isSmallPhone ? '11.5%' : isGalaxySPhone ? '9.00%' : null, 
}}>
