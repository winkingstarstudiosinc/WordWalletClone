...
function ParentSimile({onBack, commonColor, buttonColor, discoveredColor, createTextColor, addOrangeButton }) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && !notesDataLoaded) {
      ...
      loadStoredData('SimileNotes')
        .then(data => {
          setEditorContent(data || '');
          console.log('Data loaded for SimileNotes:', data);
          setNotesDataLoaded(true); // Mark data as loaded
        })
        .catch(err => console.error('Error loading SimileNotes:', err));
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    if (typeof editorContent !== 'string' || !editorContent.trim()) {
      Alert.alert(
        'Not looking so hot,',
        "we're lacking similes to save. Please chicken scratch away.",
      );
      return;
    }

    saveData('SimileNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert(
          'Bravo, las or lad!',
          'Your similes have been successfully saved!',
        );
      })
          ...
          <View style={{position: 'relative', bottom: '5%'}}>
            ...
            <View
              style={{
                ...
                borderRadius: '50%',
                left: '32.5%',
                top: isMediumTallPhone ? '22%' : isCompactMediumPhone ? '19%' : isSmallPhone ? '14%' : isGalaxySPhone ? '24%' : null,
              }}>
              <TouchableOpacity
                style={{
            ...
            <View style={{bottom: '13%'}}>
              ...
              <View
                style={{
                  position: 'relative',
                  right: '40%',
                  top: isMediumTallPhone ? '18%' : isCompactMediumPhone ? '20%' : isSmallPhone ? '16%' : isGalaxySPhone? '18%' : null,
                  bottom: '6%',
                  marginTop: '4%',
                }}>
                <View
                    ...
                    onPress={() => {
                      ...
                      saveData(classIdentifier, fusions)
                        .then(() => {
                          Alert.alert(
                            'Prim and proper!',
                            'Your simile list has been saved with great gusto!',
                          );
                          setLastSavedFusions([...fusions]);
                        })
                        .catch(() => {
                          Alert.alert(
                            'Schucks!',
                            'An error occurred while saving your simile smiles. Please give it another whirl.',
                          );
                        });
                    }}>
        ...
        <>
          <View style={{height: '104%'}}>
            <View
              ...
              <Text> Write your simile notes and thoughts here</Text>
            </View>
            <View style={{ height: isMediumTallPhone ? hp(62) : isCompactMediumPhone ? hp(60) : isSmallPhone ? hp(57) : isGalaxySPhone ? hp(66.5) : null }}>
            <DeviceQuill
            ...
            <View style={{position: 'relative', right: '35%', bottom: isMediumTallPhone ? '9.02%' : isCompactMediumPhone ? '11.5%' : isSmallPhone ? '11.5%' : isGalaxySPhone ? '9.50%' : null }}>
