...
function ParentPersonification({onBack, commonColor, buttonColor, discoveredColor, createTextColor, addOrangeButton }) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && !notesDataLoaded) {
      ...
      loadStoredData('PersonificationNotes')
        .then(data => {
          setEditorContent(data || '');
          console.log('Data loaded for PersonificationNotes:', data);
          setNotesDataLoaded(true); // Mark data as loaded
        })
        .catch(err =>
          console.error('Error loading PersonificationNotes:', err),
        );
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    if (typeof editorContent !== 'string' || !editorContent.trim()) {
      Alert.alert(
        'Look again fellow wordsmith,',
        "we're lacking personifications to save. Please feel free to jot some down!",
      );
      return;
    }

    saveData('PersonificationNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert(
          'Bravo, lassy,',
          "Your notes are retained forever more. Don't stop now!",
        );
      })
  ...
  return (
    <View style={{padding: wp(5.2), width: '110%', height: '100%'}}>
      {displayMode === 'List' ? (
          ...
          <View style={{position: 'relative', bottom: '5%'}}>
            ...
            <View
              style={{
                ...
                borderRadius: '50%',
                left: '32.5%',
                top: isMediumTallPhone ? '20%' : isCompactMediumPhone ? '19%' : isSmallPhone ? '19%' : isGalaxySPhone ? '23.0%' : null,
              }}>
              <TouchableOpacity
                style={{
            ...
            <View style={{bottom: '13%'}}>
              ...
              <View
                style={{
                  position: 'relative',
                  right: '40%',
                  top: isMediumTallPhone ? '19%' : isCompactMediumPhone ? '18%' : isSmallPhone ? '16%' : isGalaxySPhone ? '21%' : null,
                  bottom: '6%',
                  marginTop: '4%',
                }}>
                <View
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (!fusions || fusions.length === 0) {
                        Alert.alert(
                          'No personifications detected, my lady or lord,',
                          'Best get typing :)?',
                        );
                        return;
                      }
                      ...
                      saveData(classIdentifier, fusions)
                        .then(() => {
                          Alert.alert(
                            'Wallah!',
                            'Your personification list has been saved safe and sound!',
                          );
                          setLastSavedFusions([...fusions]);
                        })
                        .catch(() => {
        ...
        <>
          <View style={{ position:'relative', height: '104%'}}>
            <View
              style={{
                position: 'relative',
                padding: '2.5%',
              ...
              <Text> Write your personification notes and thoughts here</Text>
            </View>
            ...
            <DeviceQuill
              ...
              storageKey={'PersonificationNotes'}
            />
            </View>
            <View
              style={{
            ...
            <View
              style={{
                position: 'relative',
                bottom: isMediumTallPhone ? '2.5%' : isCompactMediumPhone ? '4.0%' : isSmallPhone ? '4%' : isGalaxySPhone ? '3.5%' : null,
                left: '50%',
                width: '50%',
            ...
            <View style={{ position: 'relative', right: '35%', bottom: isMediumTallPhone ? '9.0%' : isCompactMediumPhone ? '11.5%' : isSmallPhone ? '11.5%' : isGalaxySPhone ? '9.5%' : null }}>
