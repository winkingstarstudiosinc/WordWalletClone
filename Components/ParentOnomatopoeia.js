...
function ParentOnomatopoeia({onBack, commonColor, buttonColor, discoveredColor, createTextColor, addOrangeButton }) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && !notesDataLoaded) {
      ...
      loadStoredData('OnomatopoeiaNotes')
        .then(data => {
          setEditorContent(data || '');
          console.log('Data loaded for OnomatopoeiaNotes:', data);
          setNotesDataLoaded(true); // Mark data as loaded
        })
        .catch(err => console.error('Error loading OnomatopoeiaNotes:', err));
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    if (typeof editorContent !== 'string' || !editorContent.trim()) {
      Alert.alert(
        "sorry 'fraid we can't proceed",
        "we're lacking onomatopoeias to save. Please feel free to write some down.",
      );
      return;
    }

    saveData('OnomatopoeiaNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert(
          'Bravo, lassy!',
          'Your onomatopoeias are stored for safekeeping. You got anymore?',
        );
      })
          ...
          <View style={{position: 'relative', bottom: '5%'}}>
            ...
            <View
              style={{
                ...
                borderRadius: '50%',
                left: '32.5%',
                top: isMediumTallPhone ? '20%' : isCompactMediumPhone ? '21%' : isSmallPhone ? '19.5%' : isGalaxySPhone ? '20%' : null,
              }}>
              <TouchableOpacity
                style={{
            ...
            <View style={{bottom: '13%'}}>
              ...
              <View
                style={{
                  position: 'relative',
                  right: '40%',
                  top: isMediumTallPhone ? '20%' : isCompactMediumPhone ? '21%' : isSmallPhone ? '16.5%' : isGalaxySPhone ? '20%' : null,
                  bottom: '6%',
                  marginTop: '4%',
                }}>
                <View
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (!fusions || fusions.length === 0) {
                        Alert.alert(
                          'No onomatopoeias on the radar, seargent.',
                      ...
                      saveData(classIdentifier, fusions)
                        .then(() => {
                          Alert.alert(
                            'Ding ding!',
                            'Your beautiful onomatopoeia has been stored successfully!',
                          );
                          setLastSavedFusions([...fusions]);
                        })
                        .catch(() => {
                          Alert.alert(
                            'Schucks!',
                            'An error occurred while saving the spiel. Please try again.',
                          );
                        });
                    }}>
        ...
        <>
          <View style={{height: '104%'}}>
            <View
              ...
              <Text> Write your onomatopoeia notes and thoughts here</Text>
            </View>
            <View style={{ height: isMediumTallPhone ? hp(62) : isCompactMediumPhone ? hp(60) : isSmallPhone ? hp(56) : isGalaxySPhone ? hp(66) : null }}>
            <DeviceQuill
            ...
            <View
              style={{
                position: 'relative',
                bottom: isMediumTallPhone ? '2.50%' : isCompactMediumPhone ? '4.0%' : isSmallPhone ? '4%' : isGalaxySPhone ? '3.50%' : null,
                left: '50%',
                width: '50%',
            ...
            <View style={{ position: 'relative', right: '35%', bottom: isMediumTallPhone ? '9.02%' : isCompactMediumPhone ? '11.5%' : isSmallPhone ? '12.5%' : isGalaxySPhone ? '9.42%' : null }}>
