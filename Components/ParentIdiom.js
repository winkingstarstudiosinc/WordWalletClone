...
function ParentIdiom({onBack, commonColor, buttonColor, discoveredColor, createTextColor, addTurquoiseButton }) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && editorContent === '') {
      ...
      loadStoredData('IdiomNotes')
        .then(data => {
          setEditorContent(data || ''); // Set content once
          console.log('Data loaded for IdiomNotes:', data);
        })
        .catch(err => console.error('Error loading IdiomNotes:', err));
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    if (!editorContent.trim()) {
      Alert.alert(
        'Schucks,',
        'looks like there nothing to save yet in here. Can you put something down?',
      );
      return;
    }
    saveData('IdiomNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert(
          'Success!',
          'Your idiom notes are under the dome, safe and chrome!',
        );
      })
        ...
        <>
          ...
          <View
            ...
            <RadioButton.Group
              ...
              <View style={{position: 'relative', left: '30%', top: isMediumTallPhone ? '4.5%' : isCompactMediumPhone ? '4.5%' : isSmallPhone ? hp(1) : isGalaxySPhone ? '4.5%' : null }}>
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  ...
                  <RadioButton backgroundColor={'white'} value={'Fun'} />
                </View>
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  <Text
                    style={{
                  ...
                  <RadioButton backgroundColor={'white'} value={'Serious'} />
                </View>
              </View>
            </RadioButton.Group>
          </View>
          <View style={{position: 'relative', bottom: '5%'}}>
            ...
            <View>
              <TouchableOpacity
                style={{
                  ...
                  padding: isMediumTallPhone ? wp(1.9) : isCompactMediumPhone ? wp(1.75) : isSmallPhone ? wp(1.50) : isGalaxySPhone ? wp(1.9) : null,
                  borderRadius: wp(2.3),
                  borderColor: 'black',
                  borderWidth: wp(0.3),
                  alignItems: 'center',
                  width: '35%',
                  bottom: isMediumTallPhone ? '21%' : isCompactMediumPhone ? '50.5%' : isSmallPhone ? '50.5%' : isGalaxySPhone ? '10%' : null, 
                  left: '32.5%',
                }}
            ...
            <View
              style={{
                position: 'relative',
                right: '40%',
                top: isMediumTallPhone ? '18%' : isCompactMediumPhone ? '14.76%' : isSmallPhone ? wp('10.76%') : isGalaxySPhone ? '19%' : null,
                bottom: '5%',
                marginTop: '4%',
              }}>
              <View
                  ...
                  onPress={() => {
                    saveData(classIdentifier, entries)
                      .then(() => {
                        ...
                        Alert.alert(
                          'Well done wordsmith or wordsmithesse!',
                          "Your idiom list has been saved. What's next?",
                        );
                      })
              ...
              <View
                ...
                <TouchableOpacity
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    borderRadius: wp(2.3), // Matches the view's borderRadius for a cohesive look
                  }}
