...
function ParentMetaphor({onBack, commonColor, buttonColor, discoveredColor, createTextColor, addOrangeButton }) {
  ...
  useEffect(() => {
    if (displayMode === 'Notes' && !notesDataLoaded) {
      ...
      loadStoredData('MetaphorNotes')
        .then(data => {
          setEditorContent(data || '');
          console.log('Data loaded for MetaphorNotes:', data);
          setNotesDataLoaded(true); // Mark data as loaded
        })
        .catch(err => console.error('Error loading MetaphorNotes:', err));
    }
  ...
  const handleSaveEditorContent = () => {
    ...
    saveData('MetaphorNotes', editorContent)
      .then(() => {
        console.log('Notes saved successfully:', editorContent);
        Alert.alert('Ho ho ho!', 'Your metaphor notes have been saved amigo!');
      })
          ...
          <View style={{position: 'relative', bottom: '5%'}}>
            <View
              style={{
                ...
                width: '50%',
                marginBottom: '4%',
                marginTop: isMediumTallPhone ? null : isCompactMediumPhone ? hp(0.33) : isSmallPhone ? hp(0.33) : isGalaxySPhone ? null : null,
              }}>
              <DeviceDropdown
            ...
            <View
              style={{
                ...
                borderRadius: '50%',
                left: '32.5%',
                top: isMediumTallPhone ? '19.5%' : isCompactMediumPhone ? '21%' : isSmallPhone ? '19%' : isGalaxySPhone ? '21.5%' : null,
              }}>
              <TouchableOpacity
                style={{
            ...
            <View style={{bottom: '13%'}}>
              ...
              <View
                style={{
                  position: 'relative',
                  right: '40%',
                  top: isMediumTallPhone ? '19%' : isCompactMediumPhone ? '20%' : isSmallPhone ?  '15%' : isGalaxySPhone ? '20.5%' : null, 
                  bottom: '6%',
                  marginTop: '4%',
                }}>
                <View
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (!fusions || fusions.length === 0) {
                        Alert.alert(
                          'No metaphors in the treasure trove',
                      ...
                      saveData(classIdentifier, fusions)
                        .then(() => {
                          Alert.alert(
                            'Bravo!',
                            'Your metaphor list has been saved with great gusto!',
                          );
                          setLastSavedFusions([...fusions]);
                        })
                        .catch(() => {
        ...
        <>
          <View style={{height: '104%'}}>
            <View
              ...
              <Text>Write your metaphor notes and thoughts here</Text>
            </View>
            <View style={{ height: isMediumTallPhone ? hp(62) : isCompactMediumPhone ? hp(60) : isSmallPhone ? hp(55) : isGalaxySPhone ? hp(62) : null }}>
            <DeviceQuill
            ...
            <View
              style={{
                position: 'relative',
                bottom: isMediumTallPhone ? '2.50%' : isCompactMediumPhone ? '4.0%' : isSmallPhone ? '4.0%' : isGalaxySPhone ? '2.50%' : null,
                left: '50%',
                width: '50%',
